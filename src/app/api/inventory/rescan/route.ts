...
import { flashScan, deepDive, calculateBurnRate, ModelType, fetchImageWithRetry } from '@/lib/gemini';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const imageParts = fetchResults.filter((r): r is NonNullable<typeof r> => r !== null);

    if (imageParts.length === 0) {
      throw new Error('All images failed to download from storage.');
    }
    ...
    const userGuidance = {
      ...
      estimated_weight_lbs: item.weight_raw,
      price_range: item.price_range,
      condition: item.metadata?.condition || item.condition || '',
      user_notes: item.metadata?.user_notes || item.metadata?.description || item.ebay_description || ''
    };
    
    if (modelType.includes('flash')) {
      aiResult = await flashScan(imageParts, modelType as ModelType, userGuidance);
      scanCost = calculateBurnRate(modelType as ModelType, aiResult.usage);
    } else {
      aiResult = await deepDive(imageParts, modelType as ModelType, userGuidance);
      scanCost = calculateBurnRate(modelType as ModelType, aiResult.usage);
    }
