...
import { useInventory, useUserCredits } from '@/features/inventory/useInventory';
...
export default function Home() {
  ...
  const setDisabledModels = usePlan((state) => state.setDisabledModels);
  ...
  const { data: items = [], isLoading: isInvLoading, error: invError } = useInventory({ excludeStatuses: ['sold', 'deleted'] });
  ...
  React.useEffect(() => {
    ...
    if (isAdmin) {
      ...
      const disabled = modelStats
        .filter((stat: any) => stat.is_enabled === false)
        .map((stat: any) => {
          const reverseMap: Record<string, string> = {
            'gemini-flash-latest': 'flash-1.5',
            'gemini-2.5-flash': 'flash',
            'gemini-3.5-flash': 'flash-3.5',
            'gemini-pro-latest': 'pro-1.5',
            'gemini-2.5-pro': 'pro-2.5',
            'gemini-3.1-pro-preview': 'pro-3.1'
          };
          return reverseMap[stat.model_id] || stat.model_id;
        });
      setDisabledModels(disabled);
    }
  }, [modelStats, isAdmin, isStatsLoading, setDisabledModels]);
          ...
          >
            {isInvLoading ? (
              ...
              <motion.div 
                ...
                initial="hidden"
                animate="show"
                className="grid responsive-grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"
              >
