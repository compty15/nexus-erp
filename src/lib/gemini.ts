...
/**
 * Fetches an image URL with retries and exponential backoff.
 * Returns the Response object if successful, or null if all retries fail.
 */
export async function fetchImageWithRetry(url: string, retries = 3, delay = 500): Promise<Response | null> {
  for (let i = 0; i < retries; i++) {
    try {
      const res = await fetch(url);
      if (res.ok) return res;
      console.warn(`[fetchImageWithRetry] Attempt ${i + 1} failed for ${url} (status: ${res.status})`);
    } catch (err) {
      console.warn(`[fetchImageWithRetry] Attempt ${i + 1} failed for ${url} with error:`, err);
    }
    if (i < retries - 1) {
 ...
 */
export async function flashScan(
  images: { data: string; mimeType: string }[],
  modelType: ModelType = "flash-1.5",
  userGuidance?: {
    name?: string;
    brand?: string;
    category?: string;
  ...
  let prompt = `Perform a detailed analysis of this item based on the provided media.
  ...
  if (userGuidance) {
    prompt += `\n\nCRITICAL USER GUIDANCE BACKBONE (Treat these values as absolute ground truth and build all details, descriptions, and marketplace drafts around them):
- Name: ${userGuidance.name || 'N/A'}
- Brand: ${userGuidance.brand || 'N/A'}
- Category: ${userGuidance.category || 'N/A'}
- Estimated Weight: ${userGuidance.estimated_weight_lbs ? `${userGuidance.estimated_weight_lbs} lbs` : 'N/A'}
- Price Range: ${userGuidance.price_range ? `Min: $${userGuidance.price_range.min}, Max: $${userGuidance.price_range.max}` : 'N/A'}
- Condition: ${userGuidance.condition || 'N/A'}
- User Notes / Description / Flaws: ${userGuidance.user_notes || 'N/A'}

Use the specific details from this USER GUIDANCE BACKBONE as the core skeleton structure for the descriptions, platform drafts (eBay, FB, Etsy, Shopify), and metadata. For instance, if the guidance notes specific flaws or states (e.g., 'blown engine', 'missing adapter', 'pristine working condition', 'works good', etc.), you MUST incorporate those explicitly and prominently in the descriptions, condition status, and wear details.`;
  }
...
export async function deepDive(
  ...
  let prompt = `Perform high-precision analysis on this media:
...
Use the specific details from this USER GUIDANCE BACKBONE as the core skeleton structure for the descriptions, platform drafts (eBay, FB, Etsy, Shopify), wear reports, and metadata. For instance, if the guidance notes specific flaws or states (e.g., 'blown engine', 'missing adapter', 'pristine working condition', 'works good', etc.), you MUST incorporate those explicitly and prominently in the descriptions, condition status, and wear details.`;
  }
