...
import { useDeleteItem, useRemoveImage, useUpdateItem } from '@/features/inventory/useInventory';
            ...
            {isEditing ? (
              <div className="relative w-full flex items-center">
                <input 
                  ...
                  className={`w-full bg-black/40 border rounded-lg px-2 py-1 pr-7 text-xs font-black uppercase tracking-widest text-white focus:outline-none ${
                    editValues.name !== item?.name ? 'border-red-500/80 bg-red-500/10 focus:border-red-500' : 'border-blue-500/30 focus:border-blue-500'
                  }`}
                  placeholder="Item Name"
                  autoFocus
                />
                {editValues.name !== item?.name && (
                  <button 
                    onClick={() => setEditValues({ ...editValues, name: item?.name || '' })}
                    className="absolute right-2 text-red-500 hover:text-red-400 p-0.5"
                    title="Revert name change"
                  >
        ...
        <div className="flex gap-3 h-32">
          ...
          <div className="flex-1 grid grid-cols-2 gap-2 overflow-hidden">
            <div 
              ...
              className={`flex flex-col justify-center rounded-xl p-2 border transition-all ${
                !isEditing ? 'bg-white/5 border-white/5 hover:bg-white/10 cursor-pointer group/field' : 
                (editValues.brand !== item?.brand ? 'bg-red-500/10 border-red-500' : 'bg-white/5 border-white/5')
              }`}
            >
              <div className="flex justify-between items-center w-full">
                ...
                {isEditing && editValues.brand !== item?.brand && (
                  <button 
                    onClick={(e) => { e.stopPropagation(); setEditValues({ ...editValues, brand: item?.brand || '' }); }}
                    className="text-red-500 hover:text-red-400 p-0.5"
                    title="Revert brand change"
                  >
            ...
            <div 
              ...
              className={`flex flex-col justify-center rounded-xl p-2 border transition-all ${
                !isEditing ? 'bg-white/5 border-white/5 hover:bg-white/10 cursor-pointer group/field' : 
                (editValues.price !== item?.price ? 'bg-red-500/10 border-red-500' : 'bg-white/5 border-white/5')
              }`}
            >
              <div className="flex justify-between items-center w-full">
                ...
                {isEditing && editValues.price !== item?.price && (
                  <button 
                    onClick={(e) => { e.stopPropagation(); setEditValues({ ...editValues, price: item?.price || '' }); }}
                    className="text-red-500 hover:text-red-400 p-0.5"
                    title="Revert valuation change"
                  >
                    <CloseIcon className="h-2.5 w-2.5" />
                  </button>
                )}
            ...
            <div 
              ...
              className={`flex flex-col justify-center rounded-xl p-2 border transition-all ${
                !isEditing ? 'bg-white/5 border-white/5 hover:bg-white/10 cursor-pointer group/field' : 
                (editValues.category !== item?.category ? 'bg-red-500/10 border-red-500' : 'bg-white/5 border-white/5')
              }`}
            >
              <div className="flex justify-between items-center w-full">
                ...
                {isEditing && editValues.category !== item?.category && (
                  <button 
                    onClick={(e) => { e.stopPropagation(); setEditValues({ ...editValues, category: item?.category || '' }); }}
                    className="text-red-500 hover:text-red-400 p-0.5"
                    title="Revert category change"
                  >
                    <CloseIcon className="h-2.5 w-2.5" />
                  </button>
                )}
            ...
            <div 
              onClick={!isEditing ? startEditing : undefined}
              className={`flex flex-col justify-center rounded-xl p-2 border transition-all ${
                !isEditing ? 'bg-white/5 border-white/5 hover:bg-white/10 cursor-pointer' : 
                (editValues.weight_raw !== item?.weight_raw ? 'bg-red-500/10 border-red-500' : 'bg-white/5 border-white/5')
              }`}
            >
              <div className="flex justify-between items-center w-full">
                <span className="text-[7px] font-black uppercase tracking-widest text-titanium-500">Weight</span>
                {isEditing && editValues.weight_raw !== item?.weight_raw && (
                  <button 
                    onClick={(e) => { e.stopPropagation(); setEditValues({ ...editValues, weight_raw: item?.weight_raw || 0 }); }}
                    className="text-red-500 hover:text-red-400 p-0.5"
                    title="Revert weight change"
                  >
                    <CloseIcon className="h-2.5 w-2.5" />
                  </button>
                )}
              ...
              {isEditing ? (
                <input 
                  ...
                  onChange={(e) => setEditValues({ ...editValues, weight_raw: parseFloat(e.target.value) || 0 })}
                  className="w-full bg-transparent border-none p-0 text-[10px] font-bold text-white focus:ring-0"
                />
              ) : (
          ...
          {isEditing ? (
            <div className="relative w-full">
              <textarea 
                ...
                className={`w-full bg-black/40 border rounded-xl px-3 py-2 pr-7 text-[10px] text-white focus:outline-none min-h-[80px] ${
                  editValues.ebay_description !== item?.ebay_description ? 'border-red-500/80 bg-red-500/10 focus:border-red-500' : 'border-blue-500/30 focus:border-blue-500'
                }`}
                placeholder="Detailed item description..."
              />
              {editValues.ebay_description !== item?.ebay_description && (
                <button 
                  onClick={() => setEditValues({ ...editValues, ebay_description: item?.ebay_description || '' })}
                  className="absolute right-2 top-2 text-red-500 hover:text-red-400 p-0.5"
                  title="Revert description change"
                >
                  <CloseIcon className="h-3.5 w-3.5" />
                </button>
              )}
