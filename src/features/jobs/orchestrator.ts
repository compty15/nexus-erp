...
export class JobOrchestrator {
  ...
  static async startInventoryScan(files: File[], branchId: string, modelType: string, isLowQualityFallback = false): Promise<string> {
    const store = useQueueStore.getState();
    ...
    const newJob: Job = {
      ...
      status: 'pending',
      type: 'inventory_scan',
      payload: { fileCount: files.length, model: modelType, isLowQualityFallback },
    ...
    try {
      ...
      const storageResults = await Promise.all(
        files.map(async (file) => {
          ...
          if (isImage) {
            const processedFile = await compressImage(file, isLowQualityFallback);
      ...
      try {
        ...
        console.warn('Clustering API failed, using fallback:', clusterError);
        // Fallback: treat each image as its own cluster (separate item card)
        clusters = storageUrls.map((url, idx) => ({
          item_name: `Item #${(idx + 1).toString().padStart(2, '0')}`,
          indices: [idx]
        }));
        
        // Log to activity log
        await supabase.from('activity_logs').insert({
          event_type: 'cluster_fallback',
          severity: 'info',
          message: `Auto-clustering failed (${clusterError.message || 'unknown'}). Falling back to individual item cards.`,
      ...
      if (isTimeoutOrNetwork && !isLowQualityFallback) {
        console.warn('Detected timeout/network failure on high quality upload. Attempting auto-fallback to 1MB/2000px...');
        try {
          await this.cancelJob(newJob.id);
        } catch (cleanupErr) {
          console.error('Cleanup error before fallback retry:', cleanupErr);
        }
        return this.startInventoryScan(files, branchId, modelType, true);
      }
